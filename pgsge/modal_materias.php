...
{
	$x = 0;
	$SQL = "SELECT * FROM materias where sistema='".$_SESSION['sistema']."' and descricao like '%".$inputb['descricao']."%';'";
	...
	if($x == 1)
	{
		...
		swal({   
 			   title: "Atenção!",   
 			   text: "Disciplina já cadastrada, escolha outro Nome.",   
 			   timer: 1500,   
	...
	{
	   $SQL1 = "INSERT into materias(sistema,descricao,status) values('".$_SESSION['sistema']."','".$_GET['descricao']."','".$_GET['situacao']."');";
	   ...
	   if($sucesso)
	   {
		   ...
		   $(\'#modalap\').modal(\'hide\');
		   requestPage2(\'?br=atu_materias&load=1\',\'listaturmas\',\'GET\');
		   </script>');
	   }
	   else
	   {
...
{
	$SQL1 = "UPDATE materias SET descricao='".$_GET['descricao']."',status='".$_GET['situacao']."' where sistema='".$_SESSION['sistema']."' and codigo='".$codigo."';";
	$sucesso = mysqli_query($db,$SQL1);
...
{
	$x = 0;
	$SQL = "SELECT * FROM materias where sistema='".$_SESSION['sistema']."' and codigo='".$codigo."';";
	$RES = mysqli_query($db,$SQL);
	while($row = mysqli_fetch_array($RES))
	{
...
<!--Tabs with Icon example -->
			...
			$('.c_gravar').on('click',function()
		    {
				...
				else
				{
				   <? if(isset($codigo))
				   {?>
			          requestPage2('?br=modal_materias&codigo=<?=$codigo;?>&descricao='+ descricao +'&situacao='+ status +'&modal=1&ap=2','modals','GET');
				   <? } else {?>
				      requestPage2('?br=modal_materias&descricao='+ descricao +'&situacao='+ status +'&modal=1&ap=1','modals','GET');
				   <? } ?>
				}
			});
		    ...
		    {
				requestPage2('?br=modal_materias&modal=1','modals','GET');
			});
